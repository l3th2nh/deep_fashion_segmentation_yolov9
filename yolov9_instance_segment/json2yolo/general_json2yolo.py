...
def convert_coco_json(json_dir="../coco/annotations/", use_segments=False, cls91to80=False,dir="data"):
    """Converts COCO JSON format to YOLO label format, with options for segments and class mapping."""
    save_dir = make_dirs(dir=dir)  # output directory
    ...
    for json_file in sorted(Path(json_dir).resolve().glob("*.json")):
        ...
        # Create image dict
        images = {"%g" % int(x["id"]): x for x in data["images"]}
